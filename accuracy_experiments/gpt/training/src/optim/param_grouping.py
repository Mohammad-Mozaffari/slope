...
def group_parameters_for_optimizer(model, optimizer_cfg, bias_weight_decay=False,
    ...
    param_dict = {pn: p for pn, p in model.named_parameters() if p.requires_grad and "lora" not in pn}
    lora_param_dict = {pn: p for pn, p in model.named_parameters() if p.requires_grad and "lora" in pn}
    ...
    if weight_decay == 0.0 or not no_decay:
        ...
        if len(lora_param_dict) > 0:
            param_groups = [
                {"params": [param_dict[pn] for pn in sorted(list(decay))], "weight_decay": weight_decay},
                {"params": [param_dict[pn] for pn in sorted(list(no_decay))], "weight_decay": 0.0},
                {"params": [lora_param_dict[pn] for pn in sorted(list(lora_param_dict.keys()))], "weight_decay": weight_decay},
            ]
        else:
            param_groups = [
                {"params": [param_dict[pn] for pn in sorted(list(decay))], "weight_decay": weight_decay},
