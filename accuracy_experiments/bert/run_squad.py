...
def main():
    ...
    parser.add_argument("--do_eval",
                        action='store_true',
                        help="Whether to use evaluate accuracy_experiments of predictions")
    ...
    if os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train and os.listdir(
            args.output_dir) != ['logfile.txt'] and is_main_process():
    ...
    if args.pruned_matrix != "none":
        ...
        skip_layers.update({model.qa_outputs})
    ...
    for name, param in model.named_parameters():
        ...
        if param.requires_grad:
            ...
            nnz += torch.sum(param != 0).item()
    dllogger.log(step="PARAMETER", data={"num_parameters": num_params})
    dllogger.log(step="PARAMETER", data={"num_trainable_parameters": num_trainable_params})
    dllogger.log(step="PARAMETER", data={"trainable_ratio": num_trainable_params / num_params})
    dllogger.log(step="PARAMETER", data={"nnz": nnz})
    dllogger.log(step="PARAMETER", data={"sparsity": 1 - nnz / num_params})
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            desc = ("Epoch: %d - Global Step: %d, Average Loss: %.4f, Learning Rate: %.4e" %
                    (0, 0, 0., 0.))
            train_iter = tqdm(train_dataloader, disable=args.disable_progress_bar,
                              desc=desc) if is_main_process() else train_dataloader
            for step, batch in enumerate(train_iter):
                ...
                if average_loss == None:
                    ...
                    average_loss = 0.01 * loss.item() + 0.99 * average_loss

                if is_main_process():
                    desc = ("Epoch: %d - Global Step: %d, Average Loss: %.4f, Learning Rate: %.4e" %
                            (epoch, global_step, average_loss, optimizer.param_groups[0]['lr']))
                    train_iter.set_description(desc, refresh=True)
    ...
    if args.do_train:
        ...
        if args.max_steps == -1:
            dllogger.log(step=tuple(), data={"e2e_train_time": time_to_train,
                                             "training_sequences_per_second": len(
                                                 train_features) * args.num_train_epochs / time_to_train,
