...
class BertSelfAttention(nn.Module):
    ...
    def fix_context_size(self, context_layer):
    ...
    def forward(self, hidden_states, attention_mask):
        mixed_query_layer = timer("Linear Layer", self.query, hidden_states)
        mixed_key_layer = timer("Linear Layer", self.key, hidden_states)
        mixed_value_layer = timer("Linear Layer", self.value, hidden_states)

        query_layer = timer("Transposition", self.transpose_for_scores, mixed_query_layer)
        key_layer = timer("Transposition", self.transpose_key_for_scores, mixed_key_layer)
        value_layer = timer("Transposition", self.transpose_for_scores, mixed_value_layer)

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = timer("Attention Matmul - QK^T", torch.matmul, query_layer, key_layer)
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
        attention_scores = attention_scores + attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs = timer("Attention SoftMax", F.softmax, attention_scores, dim=-1)

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        attention_probs = timer("Dropout", self.dropout, attention_probs)

        context_layer = timer("Attention Matmul - AV", torch.matmul, attention_probs, value_layer)
        context_layer = timer("Transposition", self.fix_context_size, context_layer)
...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        hidden_states = timer("Linear Layer", self.dense, hidden_states)
        hidden_states = timer("Dropout", self.dropout, hidden_states)
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        pooled_output = timer(fused + "Linear Layer", self.dense_act, first_token_tensor)
...
class BertPredictionHeadTransform(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = timer(fused + "Linear Layer", self.dense_act, hidden_states)
        hidden_states = timer("Layer Norm", self.LayerNorm, hidden_states)
        return hidden_states
...
class BertLMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = timer("Linear Layer", self.decoder, hidden_states) + self.bias
        return hidden_states
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output=None):
        ...
        if self.seq_relationship is not None:
            seq_relationship_score = timer("Linear Layer", self.seq_relationship, pooled_output)
        else:
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask):
        ...
        embedding_output = timer("Embedding", self.embeddings, input_ids, token_type_ids)
