...
from src.utils import (is_main_process, mkdir_by_main_process, format_step,
                   get_world_size)
from processors.glue import PROCESSORS, convert_examples_to_features, output_modes
...
def f1_score(y_true, y_pred):
    ...
    TP = torch.sum((y_true == 1) & (y_pred_binary == 1))
    FP = torch.sum((y_true == 0) & (y_pred_binary == 1))
    FN = torch.sum((y_true == 1) & (y_pred_binary == 0))

    # Calculate Precision and Recall
    precision = TP / max((TP + FP), 1e-8)  # To avoid division by zero
    recall = TP / max((TP + FN), 1e-8)  # To avoid division by zero

    # Calculate F1 score
    result = 2 * (precision * recall) / max((precision + recall), 1e-8)  # To avoid division by zero
...
def matthews_corrcoef(y_true, y_pred):
    ...
    y_pred = torch.tensor(y_pred)
    # Convert predictions to binary values (0 or 1)
    y_pred_binary = torch.tensor(y_pred > 0.5).float()

    # True Positives, True Negatives, False Positives, False Negatives
    TP = torch.sum((y_true == 1) & (y_pred_binary == 1))
    TN = torch.sum((y_true == 0) & (y_pred_binary == 0))
    FP = torch.sum((y_true == 0) & (y_pred_binary == 1))
    FN = torch.sum((y_true == 1) & (y_pred_binary == 0))

    # Calculate Matthews correlation coefficient
    mcc = (TP * TN - FP * FN) / torch.sqrt((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN) + 1e-8)  # To avoid division by zero
...
def compute_metrics(task_name, preds, labels):
    ...
    if task_name == "cola":
        return {"mcc": matthews_corrcoef(preds, labels)}
...
def parse_args(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('--log_freq', # added this argument to track loss to see if it converges
                        type=int, default=100,
                        help='frequency of logging loss.')
    parser.add_argument('--pruned_matrix', 
                        default="none", 
                        type=str,
                        help="The matrix to prune")
    parser.add_argument('--skip_first_block', 
                        default="False", 
                        type=str,
                        help="Skip the first block when pruning")
    parser.add_argument('--skip_last_block', 
                        default="False", 
                        type=str,
                        help="Skip the last block when pruning")
    parser.add_argument('--skip_attention', 
                        default="False", 
                        type=str,
                        help="Skip the attention block when pruning")
    parser.add_argument('--reduction_dim', 
                        default="True", 
                        type=str,
                        help="Apply pruning on the reduction dimension")
    parser.add_argument('--add_lora', 
                        default="False", 
                        type=str,
    ...
    parser.add_argument('--sparsity_increment',
                        type=str, 
                        default="",
...
def get_train_features(data_dir, bert_model, max_seq_length, do_lower_case,
    ...
    try:
        ...
        logger.info("\nLoaded pre-processed features from {}\n".format(
...
def main(args):
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        n_gpu = 1 # this line was previously n_gpu = torch.cuda.device_count(), changed to avoid potential bugs
    else:
    ...
    if args.pruned_matrix != "none":
        ...
        skip_layers.update({model.classifier}) # output head of BertForSequenceClassification
        prune_model(model, skip_layers=skip_layers, pruned_matrix=args.pruned_matrix, reduction_dim=args.reduction_dim, add_lora=args.add_lora, lora_rank=args.lora_rank,unstructured_masking = args.unstructured_masking, unstructured_mask_sparsity = args.unstructured_mask_sparsity)
    ...
    if args.do_train:
        ...
        # train_data = gen_tensor_dataset(output_mode, train_features)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(
                ...
                final_loss = loss.item() # added this line from lines 757-760 from run_glue.py, to track if it is converging
                if step % args.log_freq == 0:
                    logger.info("  Iteration = %d", step)
                    logger.info("  Training loss = %f", final_loss)
    ...
    if (args.do_eval or args.do_predict) and is_main_process():
        ...
        for i, (input_ids, input_mask, segment_ids, label_ids, seq_lengths) in tqdm(
        ...
        if args.do_eval:
            ...
            if output_mode == 'classification':
                ...
                eval_result = compute_metrics(args.task_name, preds, eval_labels)
