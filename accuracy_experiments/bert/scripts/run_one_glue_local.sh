...
batch_size=4
out_dir="results/checkpoints-glue"
data_dir="glue/CoLA"
...
learning_rate=${2:-"2.4e-5"} # Pls note that some lr can lead to CoLA mcc goes to 0
warmup_proportion=${3:-"0.1"}
epochs=${4:-"3"} # For MRPC, change it to 5.
max_steps=${5:-"-1.0"}
precision=${6:-"fp16"}
seed=${7:-"0"}
mode=${8:-"train eval prediction"} # added prediction
